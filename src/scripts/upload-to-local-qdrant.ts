...
async function createCollection(): Promise<void> {
  try {
    console.log('Starting local collection creation process...');
 ...
 */
async function uploadToLocalQdrant() {
  try {
    console.log('Starting local vectorization process...');
    ...
    console.log('Storing embeddings in local Qdrant...');
    await addVectors(embeddings, ids, payloads);

    console.log('Local vectorization process completed successfully!');
  } catch (error) {
...
uploadToLocalQdrant().then(() => {
  console.log('Upload to local Qdrant completed.');
}).catch(error => {
  console.error('Error in local upload process:', error);
}); 